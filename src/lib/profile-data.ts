import type { ProfileData } from './types';

export const profileData: ProfileData = {
  personalInfo: {
    name: "Vishnu Prajapat",
    title: "AI Full Stack Developer",
    location: "India",
    avatar: "/images/my.png",
    aboutAvatar: "/images/about.png",
    email: "vishanuprajapati888@gmail.com",
    phone: "+91 6377749427",
    linkedin: "https://www.linkedin.com/in/vishnu-prajapat-a7b240293/",
    github: "https://github.com/vishnu098prajapat",
    portfolioUrl: "https://vishnu-portfolio.vercel.app",
    resumeUrl: "#", 
  },
  summary:
    "Full Stack Web Developer with AI hands-on experience in designing and developing modern web applications using React, Node.js, and Firebase.",
  bio: "Full Stack Web Developer with AI hands-on experience in designing and developing modern web applications using HTML, CSS, JavaScript, React.js, APIs, and Firebase. Strong understanding of frontend architecture, responsive UI development, and backend integration with authentication and databases. Basic understanding of Machine Learning concepts including supervised and unsupervised learning.",
  skills: [
    { name: "React.js", endorsement: 25 },
    { name: "C++", endorsement: 20 },
    { name: "Python", endorsement: 18 },
    { name: "DSA", endorsement: 15 },
    { name: "Firebase", endorsement: 14 },
    { name: "Automation", endorsement: 12 },
    { name: "AI & GenAI", endorsement: 15 },
    { name: "Data Science Libs", endorsement: 10 },
  ],
  projects: [
    {
      name: "Sparrowfy",
      description: "A comprehensive SaaS platform for coaching management, optimizing student tracking and scheduling.",
      technologies: [{ name: "React.js" }, { name: "API" }, { name: "SaaS" }],
      imageUrl: "/images/sparrowfy.png",
      imageHint: 'coaching management',
      projectUrl: "https://aiquizcraft.com",
      githubUrl: "#",
    },
    {
      name: "Insta Automation",
      description: "A powerful automation tool for Instagram to streamline interactions and manage social media workflows efficiently.",
      technologies: [{ name: "Automation" }, { name: "Python" }, { name: "Web" }],
      imageUrl: "/images/insta_automation.png",
      imageHint: 'instagram automation',
      projectUrl: "https://insta-automation-gray.vercel.app/",
      githubUrl: "#",
    },
    {
      name: "Gemini Prompts",
      description: "Advanced AI tool for image editing and creative prompt generation using Gemini API for high-quality results.",
      technologies: [{ name: "React" }, { name: "Gemini AI" }, { name: "Image Editing" }],
      imageUrl: "/images/gemini_prompts.png",
      imageHint: 'ai prompts tool',
      projectUrl: "https://gemini-prompts-flax.vercel.app/",
      githubUrl: "#",
    },
    {
      name: "ChatWithPDF",
      description: "AI-powered tool to interact with PDFs, featuring Video Summarizer, Viva with AI, and natural language extraction.",
      technologies: [{ name: "React" }, { name: "AI Chatbot" }, { name: "Summarizer" }],
      imageUrl: "/images/chatWithpdf.png",
      imageHint: 'ai chat pdf',
      projectUrl: "https://chat-with-pdf-five-tau.vercel.app/",
      githubUrl: "#",
    },
    {
      name: "Image & PDF Tool",
      description: "A versatile toolkit for processing images and PDFs, including conversion and editing features.",
      technologies: [{ name: "React" }, { name: "Utility" }, { name: "Tools" }],
      imageUrl: "/images/image_pdf_tool.png",
      imageHint: 'pdf utility tool',
      projectUrl: "https://studio--studio-7834146289-df62e.us-central1.hosted.app/",
      githubUrl: "#",
    },
    {
      name: "Resume Builder",
      description: "A professional web application for creating resumes easily with customizable templates and AI assistance.",
      technologies: [{ name: "React" }, { name: "Web App" }, { name: "AI" }],
      imageUrl: "/images/make_resume.png",
      imageHint: 'resume builder',
      projectUrl: "https://resume-builder-ten-black.vercel.app/",
      githubUrl: "#",
    },
    {
      name: "Online Gaming Platform",
      description: "A modern online gaming platform featuring a responsive interface and real-time state management.",
      technologies: [{ name: "Vite" }, { name: "React" }],
      imageUrl: "/images/Game.png",
      imageHint: 'online gaming',
      projectUrl: "http://realonlinegaming.netlify.app",
      githubUrl: "#",
    },
    {
      name: "Watermelon GSAP",
      description: "A creative landing page showcasing advanced GSAP scroll animations and interactive UI elements.",
      technologies: [{ name: "GSAP" }, { name: "Animation" }, { name: "UI" }],
      imageUrl: "/images/w.jpeg",
      imageHint: 'scroll animation',
      projectUrl: "https://watermelon-gsap.vercel.app/",
      githubUrl: "#",
    },
  ],
  experience: [
    {
      title: "Freelance AI Full Stack Developer",
      company: "Self-Employed",
      location: "Remote",
      startDate: "Jan 2023",
      endDate: "Present",
      description:
        "Building end-to-end web applications with AI integrations, focusing on automation and high-quality user experiences.",
    },
  ],
  education: [
    {
      degree: "Bachelor of Technology",
      institution: "CS Engineering",
      location: "India",
      startDate: "2021",
      endDate: "2025",
      description: "Focusing on Computer Science and Engineering.",
    },
  ],
};